...
const ViewEventModal = ({ isOpen, onClose, events }) => {
  return (
    <Modal
      ...
      className="w-[40rem] bg-yellow left-[8%] py-6 absolute flex justify-start items-center rounded-xl flex-col gap-[0.8rem] h-full"
    >
      ...
      <table className="w-full">
        <thead>
          <tr>
            <th className="font-bold flex items-center justify-center">Title</th>
        ...
        <tbody className="w-full">
          {events.map((event) => (
            <tr key={event.id} className="border-b border-gray-400 text-center">
              ...
              <td className="text-center">{moment(event.start).format("DD-MM-YYYY, HH:mm")}</td>
              <td className="text-center">{moment(event.end).format("DD-MM-YYYY, HH:mm")}</td>
            </tr>
          ))}
        </tbody>
      </table>
